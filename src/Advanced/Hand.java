...
import Advanced.Card.CompareDescending;
import Advanced.Card.CompareSuit;
...
public class Hand implements Iterable<Card>, Serializable {
    ...
    public void add(Hand h) {
        ...
        for (Card c : h) {
            //Taking the rank and suit of the given card as integer values which 
            //will be used to specify the place in the counter array to be added
            //to
            int pos = c.getRank().cardRankValue - 2;
            int suit = c.getSuit().ordinal();
            hand.add(c);
    ...
    public boolean remove(Card c) {
        ...
        for (Card temp : hand) {
            ...
            if (temp == c) {
                //Taking the rank and suit of the given card as integer values 
                //which will be used to specify the place in the counter array 
                //to be taken away from
                //-2 as rank 2 is 0 in the array
                int pos = c.getRank().cardRankValue - 2;
                int suit = c.getSuit().ordinal();
